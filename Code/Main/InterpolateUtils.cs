...
namespace Flowframes.Main
{
    class InterpolateUtils
    {
        ...
        public static void RemoveCloseSceneChanges(string scenesPath, string sceneScoresFilePath, Fraction fps)
        {
            ...
            if (!Directory.Exists(scenesPath) || !File.Exists(sceneScoresFilePath))
                return;

            // Read scores file generated by ffmpeg scene detection
            string[] sceneScoreLines = File.ReadAllLines(sceneScoresFilePath);
            ...
            foreach (string line in sceneScoreLines)
            {
                ...
                {
                    float score = line.Split('=').Last().GetFloat();
