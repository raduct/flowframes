...
namespace Flowframes.Main
{
    class InterpolateUtils
    {
        ...
        public static void RemoveConsecutiveFrames(string scenesPath, string sceneScoresFile, Fraction fps)
        {
            Logger.Log("Remove consecutive scene changes...");

            if (!Directory.Exists(scenesPath) || !File.Exists(sceneScoresFile))
                return;

            // Read scores file generated by ffmpeg scene detection
            string[] sceneScoreLines = File.ReadAllLines(sceneScoresFile);
            Dictionary<int,float> sceneScore = new Dictionary<int,float>();
            float score = 0;
            int frameNo = 0;
            foreach (string line in sceneScoreLines)
            {
                if (line.StartsWith("frame:"))
                    frameNo = line.Split("pts:").Last().Split("pts_time:").First().GetInt();
                else if (line.StartsWith("lavfi.scene_score"))
                {
                    score = line.Split('=').Last().GetFloat();
                    sceneScore.Add(frameNo, score);
                }
            }

            // Keep only the scene change with maximum score in the time range of 400ms
            List<int> framesToDelete = new List<int>();
            int range = fps.GetFloat() > 0 ? (int)Math.Floor(fps.GetFloat() * 0.4 + 0.5) : 10;
            while (sceneScore.Count > 0)
            {
                float maxScore = 0;
                int maxScoreIndex = -1;
                foreach (int frameIndex in sceneScore.Keys)
                    if (maxScore < sceneScore[frameIndex])
                    {
                        maxScore = sceneScore[frameIndex];
                        maxScoreIndex = frameIndex;
                    }
                for (int i = maxScoreIndex - range; i <= maxScoreIndex + range; i++)
                    if (sceneScore.ContainsKey(i))
                    {
                        if (i != maxScoreIndex)
            ...
            foreach (int frameIndex in framesToDelete)
                IoUtils.TryDeleteIfExists(Path.Combine(scenesPath, frameIndex.ToString().PadLeft(Padding.inputFrames, '0') + I.currentSettings.framesExt));

            Logger.Log($"Removed {framesToDelete.Count} scene {(framesToDelete.Count == 1 ? "change" : "changes")}.".Replace(" 0 ", " no "));
        }
